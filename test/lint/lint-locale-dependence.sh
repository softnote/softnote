...
# Be aware that softnoted and softnote-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in softnoted.
#
...
# C locale.
#
# In contrast, softnoted does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where softnoted is guaranteed to be running
# with the classic locale ("C") whereas the locale of softnote-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in softnoted but
# "1,230000" in softnote-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/softnote-tx.cpp.*stoul"
    "src/softnote-tx.cpp.*trim_right"
