...
The Softnote Core daemon can be configured to act as a trusted "border
router", implementing the softnote wire protocol and relay, making
...
The ZeroMQ feature in Softnote Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building softnoted:
...
For instance:
    ...
    $ softnoted -zmqpubhashtx=tcp://127.0.0.1:217590 \
               -zmqpubhashtx=tcp://192.168.1.2:217590 \
               -zmqpubrawtx=ipc:///tmp/softnoted.tx.raw \
...
These options can also be provided in softnote.conf.
...
From the perspective of softnoted, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into softnoted directly. Furthermore, no information is
...
using. Softnoted appends an up-counting sequence number to each
