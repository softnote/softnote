# Fuzzing Softnote Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Softnote Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/softnote/softnote
$ cd softnote/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/softnote/softnote/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/softnote/softnote/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/softnote/softnote/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/softnote/softnote/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`softnote-core/qa-assets`](https://github.com/softnote-core/qa-assets) repo.

To fuzz `process_message` using the [`softnote-core/qa-assets`](https://github.com/softnote-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/softnote-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`softnote-core/qa-assets`](https://github.com/softnote-core/qa-assets) repo.

Every single pull request submitted against the Softnote Core repo is automatically tested against all inputs in the [`softnote-core/qa-assets`](https://github.com/softnote-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Softnote Core more robust.
...
with certain assembly code from Softnote Core's code. See [developer notes on sanitizers](https://github.com/softnote/softnote/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Softnote Core using afl++

## Quickstart guide

To quickly get started fuzzing Softnote Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Softnote Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Softnote Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Softnote Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Softnote
Core without having to write any custom fuzzing harness. The `softnoted` server
...
```sh
$ mkdir softnote-honggfuzz-p2p/
$ cd softnote-honggfuzz-p2p/
$ git clone https://github.com/softnote/softnote
$ cd softnote/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/softnoted.cpp b/src/softnoted.cpp
...
EOF
$ make -C src/ softnoted
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=17444 -f inputs/ -- \
          src/softnoted -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:17444 -logthreadnames \
                       -debug
```

# OSS-Fuzz

Softnote Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/softnote-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=softnote-core).
...
because Softnote is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_softnote-core/latest).
