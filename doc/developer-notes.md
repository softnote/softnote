...
- [Development guidelines](#development-guidelines)
    - [General Softnote Core](#general-softnote-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Softnote Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
with "play softnotes" on a test network.
...
Softnote Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/softnote/softnote/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_softnote
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_softnote --log_level=test_suite
$ valgrind -v --leak-check=full src/softnoted -printtoconsole
...
# A coverage report will now be accessible at `./test_softnote.coverage/index.html`.
```
...
To profile a running softnoted process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep softnoted` -- sleep 60
```
...
Softnote Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/softnote/softnote/issues/12691)
...
- [Main thread (`softnoted`)](https://doxygen.bitcoincore.org/softnoted_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `softnoted.cpp`. Responsible for starting up and
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 17605.
...
However, in open source software such as Softnote Core, where everyone uses
...
pay attention to for reviewers of Softnote Core code.

General Softnote Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Softnote Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `SOFTNOTE_FOO_BAR_H`, e.g.

```c++
#ifndef SOFTNOTE_FOO_BAR_H
#define SOFTNOTE_FOO_BAR_H
...
#endif // SOFTNOTE_FOO_BAR_H
```
...
Some of these are maintained by active developers of Softnote Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Softnote Core so that they can be integrated
...
- src/secp256k1
  - Upstream at https://github.com/softnote-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/softnote-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/softnote-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Softnote's `select()` loop, because
...
compatibility between nodes. This happened in Softnote 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Softnote's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/softnote/softnote/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/softnote/softnote/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/softnote/softnote/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `softnote-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `softnote-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Softnote Core, the wallet was always
...
- For consistency and friendliness to code generation tools, interface method
  ...
  before in [#14635](https://github.com/softnote/softnote/pull/14635).
