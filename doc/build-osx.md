...
This guide describes how to build softnoted, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Softnote Core from source.
...
### 4. Clone Softnote repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Softnote Core repository to a directory.
...
It is not necessary to build wallet functionality to run `softnoted` or  `softnote-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/softnote/softnote/blob/master/doc/descriptors.md), `sqlite` is required.
...
Softnote Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/softnote/softnote/issues/7714).
...
You can deploy a `.dmg` containing the Softnote Core application using `make deploy`.
...
## Building Softnote Core

### 1. Configuration

There are many ways to configure Softnote Core, here are a few common examples:
...
Run the following in your terminal to compile Softnote Core:
...
## Running Softnote Core

Softnote Core should now be available at `./src/softnoted`.
If you compiled support for the GUI, it should be available at `./src/qt/softnote-qt`.

The first time you run `softnoted` or `softnote-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Softnote/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Softnote"

touch "/Users/${USER}/Library/Application Support/Softnote/softnote.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Softnote/softnote.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Softnote/debug.log
```

## Other commands:

```shell
./src/softnoted -daemon      # Starts the softnote daemon.
./src/softnote-cli --help    # Outputs a list of command-line options.
./src/softnote-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/softnote-qt -server # Starts the softnote-qt server mode, allows softnote-cli control
```
