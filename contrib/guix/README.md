# Bootstrappable Softnote Core Builds

This directory contains the files necessary to perform bootstrappable Softnote
Core builds.
...
To build Softnote Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Softnote Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/softnote-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/softnote-core/guix.sigs).
