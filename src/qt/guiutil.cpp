...
#include <qt/softnoteaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Softnote address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new SoftnoteAddressEntryValidator(parent));
    widget->setCheckValidator(new SoftnoteAddressCheckValidator(parent));
}
...
bool parseSoftnoteURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no softnote: URI
    if(!uri.isValid() || uri.scheme() != QString("softnote"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SoftnoteUnits::parse(SoftnoteUnits::SFN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSoftnoteURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseSoftnoteURI(uriInstance, out);
}

QString formatSoftnoteURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("softnote:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SoftnoteUnits::format(SoftnoteUnits::SFN, info.amount, false, SoftnoteUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openSoftnoteConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", SOFTNOTE_CONF_FILENAME));
    ...
    /* Open softnote.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Softnote.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Softnote (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Softnote (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Softnote*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "softnote.desktop";
    return GetAutostartDir() / strprintf("softnote-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a softnote.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Softnote\n";
        else
            optionFile << strprintf("Name=Softnote (%s)\n", chain);
